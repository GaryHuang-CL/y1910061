<?php
	...
	function login()
	{
		...
		// check if need login
		$ret = preg_match('/<img id="n1"/', $result);
		...
		if(!$ret){
			...
			$ret = preg_match('/<input class="fm fm110" type="text" name="([a-z0-9]+)" value=".*?" maxlength="15"> <span class="e f7"><\/span>/', $result, $matches);
			...
			$ret = preg_match('/<input class="fm fm110" type="password" name="([a-z0-9]+)" value=".*?" maxlength="20"> <span class="e f7"><\/span>/', $result, $matches);
			...
			$postfields = 'w=1152%3A864'
						...
						. '&s1.x=&s1.y=&s1=login&autologin=ja';
