...
function MissionCaseAttack ($FleetRow)
{
	...
	if ($FleetRow['fleet_start_time'] <= time()) {
		if ($FleetRow['fleet_mess'] == 0) {
			...
			// What remains of the attacker
			$CurrentSet = $walka["atakujacy"];
			// What remains of the attacked
			$TargetSet = $walka["wrog"];
			// The result of the battle
			$FleetResult = $walka["wygrana"];
			// Long report (battle report details)
			$dane_do_rw = $walka["dane_do_rw"];
			// Short report (cdr + units lost)
			$zlom = $walka["zlom"];
			...
			// If the little funny that sent the fleet had put resources ...
			...
			// Updating the field of the planet to ruin attacked
			...
			// Here we will discuss the piece of fat to see if there is a way to have a moon!
			...
			// Colorization of the summary report for the attacker
			...
			// Addition of small point stiffness
			...
			// Adding a point to counter raids
